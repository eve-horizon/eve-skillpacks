...
The derived name is prefixed with the registry host at build time (e.g., `registry.eve.example.com/api:sha-abc123`). You can still set `image` explicitly to override the default.
